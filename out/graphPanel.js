...
exports.GraphViewProvider = exports.GraphPanel = exports.GitGraphContentProvider = void 0;
...
exports.GitGraphContentProvider = GitGraphContentProvider;
class GraphPanel {
    constructor(panel, extensionUri, repoPath) {
        this.disposables = [];
        this.panel = panel;
        ...
        const watcher = vscode.workspace.createFileSystemWatcher(new vscode.RelativePattern(gitDir, '{HEAD,MERGE_HEAD,index,refs/heads/**,refs/remotes/**}'));
        ...
        this.panel.webview.onDidReceiveMessage(async (msg) => {
            if (msg.command === 'ready') {
                ...
                try {
                    const files = await (0, gitService_1.getCommitFiles)(this.repoPath, msg.hash);
                ...
                try {
                    await execFileAsync('git', ['-C', this.repoPath, 'add', '--', msg.path]);
                }
                catch { }
                await this.refresh();
            }
                ...
                try {
                    await execFileAsync('git', ['-C', this.repoPath, 'stash', 'pop', msg.name]);
                }
                catch { }
                await this.refresh();
            }
            else if (msg.command === 'dropStash') {
                const confirm = await vscode.window.showWarningMessage(`Delete ${msg.name}? This cannot be undone.`, { modal: true }, 'Delete');
                if (confirm === 'Delete') {
                    try {
                        await execFileAsync('git', ['-C', this.repoPath, 'stash', 'drop', msg.name]);
                    }
                    catch { }
                    await this.refresh();
                }
    ...
    static createOrShow(extensionUri, repoPath) {
        ...
        const panel = vscode.window.createWebviewPanel('gitGraph', 'Open Git', column || vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    async refresh() {
        try {
            const data = await (0, gitService_1.getGitLog)(this.repoPath);
    ...
    async openDiff(hash, parentHash, filePath, status) {
        ...
        const makeUri = (ref) => vscode.Uri.parse(`git-graph:/${encodeURIComponent(fileName)}?${encodeURIComponent(JSON.stringify({ repoPath: this.repoPath, ref, filePath }))}`);
...
function scanWorkspaceForGitRepos() {
    const results = [];
...
class GraphViewProvider {
    constructor(extensionUri, repoPath, repoPaths = []) {
        this.extensionUri = extensionUri;
        this.repoPath = repoPath;
        this.repoPaths = repoPaths;
        this.disposables = [];
    }
    get currentRepoPath() { return this.repoPath; }
    setRepoPaths(paths) {
    ...
    liveRepoPaths() {
        ...
        if (gitExt?.isActive) {
            ...
            if (api.repositories.length > 0) {
                const live = api.repositories.map((r) => r.rootUri.fsPath);
                this.repoPaths = live;
                return live;
            }
        }
        ...
        if (msg.command === 'ready') {
            ...
            try {
                ...
                if (aheadCount > 0) {
                    ...
                    const choice = await vscode.window.showWarningMessage(`Local '${msg.branch}' is ${aheadCount} ${word} ahead of origin. Reset to match origin/${msg.branch}?`, { modal: true }, 'Reset to Origin', 'Keep Local');
            ...
            try {
                ...
                this.sendToast('success', `Checked out ${msg.hash.slice(0, 8)}`);
            }
            catch (err) {
                this.sendToast('error', 'Checkout Failed', (err.stderr || err.stdout || err.message || '').trim().split('\n')[0]);
            }
            ...
            if (msg.mode === 'hard') {
                const confirm = await vscode.window.showWarningMessage(`Hard reset to ${msg.hash.slice(0, 8)}? All uncommitted changes will be discarded.`, { modal: true }, 'Reset');
                if (confirm !== 'Reset') {
                    return;
                }
            }
            ...
            const confirm = await vscode.window.showWarningMessage(`Delete local branch "${msg.branch}"?`, { modal: true }, 'Delete');
            if (confirm !== 'Delete') {
                return;
            }
            try {
                ...
                if (detail.includes('not fully merged')) {
                    const force = await vscode.window.showWarningMessage(`Branch "${msg.branch}" is not fully merged. Force delete?`, { modal: true }, 'Force Delete');
            ...
            const confirm = await vscode.window.showWarningMessage(`Delete remote branch "origin/${msg.branch}"?`, { modal: true }, 'Delete');
            if (confirm !== 'Delete') {
                return;
            }
            try {
            ...
            const confirm = await vscode.window.showWarningMessage(`Rebase current branch onto ${msg.hash.slice(0, 8)}? This rewrites commit history.`, { modal: true }, 'Rebase');
            if (confirm !== 'Rebase') {
                return;
            }
            this.showLoading();
            ...
            const confirm = await vscode.window.showWarningMessage(`Delete tag "${msg.name}"?`, { modal: true }, 'Delete');
            if (confirm !== 'Delete') {
                return;
            }
            try {
            ...
            const confirm = await vscode.window.showWarningMessage('Abort the current merge? All merge changes will be discarded.', { modal: true }, 'Abort Merge');
...
GraphViewProvider.viewType = 'git-graph.panelView';
function getNonce() {
    let text = '';
